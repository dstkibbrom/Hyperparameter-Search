...
data_set=pd.read_csv("/home/tf/All_ids/ID_prediction/IDS_arbID/two_fold_LSTM/second_fold/ID_sequence.csv",
                     index_col=False,skiprows=1, names=["arb_id"], sep=",")
...
def create_dataset(encoded_data,seq_length, batch_size, buffer_size):
...
HP_BATCH_SIZE = hp.HParam('batch_size', hp.Discrete([10,20,30]))
...
HP_RNN_DROPOUT = hp.HParam('RNN_dropout', hp.Discrete([0.0,0.1, 0.3, 0.5]))
HP_KEEP_PROB = hp.HParam('keep_prob', hp.Discrete([0.0,0.1, 0.3,0.5]))
HP_GRAD_CLIP= hp.HParam('grad_clip', hp.Discrete([1, 2,3]))

HP_EXP_LR=hp.HParam('exp_lr', hp.Discrete([0, 1]))
HP_LR=hp.HParam('lr', hp.Discrete([0.001,0.01,0.1]))
HP_MOMENTUM=hp.HParam('momentum', hp.Discrete([0.7,0.8,0.9]))
HP_BUFFER_SIZE=hp.HParam('buffer_size', hp.Discrete([1000]))
...
with tf.summary.create_file_writer('logs/hparam_tuning').as_default():
    ...
    hparams=[HP_SEQ_LEN,HP_BATCH_SIZE,HP_RNN_TYPE, 
             HP_RNN_NLAYERS,HP_RNN_UNITS,HP_RNN_DROPOUT,
             HP_KEEP_PROB, HP_GRAD_CLIP,HP_EXP_LR,
             HP_LR,HP_MOMENTUM, HP_BUFFER_SIZE,HP_EMBED_DIM, HP_STATE],
...
def build_model(hparams):
    ...
    if hparams[HP_RNN_TYPE]=='GRU':
        for i in range(hparams[HP_RNN_NLAYERS]):
            model.add(tf.keras.layers.GRU(hparams[HP_RNN_UNITS], 
                                          ...
                                          recurrent_dropout=hparams[HP_RNN_DROPOUT], 
                                          return_sequences=True,
                                          stateful=hparams[HP_STATE]))
        
    elif hparams[HP_RNN_TYPE]=='LSTM':
        for i in range(RNN_nlayers):
        ...
        for i in range(RNN_nlayers):
            model.add(tf.keras.experimental.PeepholeLSTMCell(hparams[HP_RNN_UNITS], 
                                          kernel_initializer='glorot_uniform',
                                          recurrent_dropout=hparams[HP_RNN_DROPOUT], 
                                          return_sequences=True,
                                          stateful=hparams[HP_STATE]))

    model.add(tf.keras.layers.Dropout(hparams[HP_KEEP_PROB]))        
    model.add(tf.keras.layers.Dense(total_ids))
    if hparams[HP_EXP_LR]:
    ...
    train_ds=create_dataset(encoded[:int(len(encoded)*0.9)],
                            hparams[HP_SEQ_LEN],hparams[HP_BATCH_SIZE],hparams[HP_BUFFER_SIZE])
    val_ds=create_dataset(encoded[int(len(encoded)*0.9):],
                            hparams[HP_SEQ_LEN],hparams[HP_BATCH_SIZE],hparams[HP_BUFFER_SIZE])
    history = model.fit(train_ds, validation_data=val_ds, epochs=100,verbose=0, callbacks=[early_stopping_cb])
    return min(history.history['val_loss'][-10:])
